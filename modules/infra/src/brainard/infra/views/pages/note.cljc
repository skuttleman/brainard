(ns brainard.infra.views.pages.note
  ...
  (:require
    ...
    [brainard.schedules.infra.model :as sched.model]
...
(defn ^:private schedule-display [form-data]
  (when-let [parts (seq (->> form-data
                             (sort-by key)
                             reverse
                             (keep sched.model/->schedule-part)
                             (interpose [:span "AND"])))]
