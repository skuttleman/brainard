...
(defmethod yast/command-handler :resources/submit!
  ...
  (let [resource (colls/wrap-vector resource-id)
        ...
        {:keys [handler route-params] :as q} (yast/query db [:routing/route])]
    (emit [:resources/submit resource-id])
...
(defmethod yast/command-handler :toasts/success
  [store _ [_ {:keys [message]}] _]
  (store/dispatch! store [:toasts/create! :success message]))

(defmethod yast/command-handler :toasts/failure
  [store _ [_ errors] _]
